 ...
 * @author admin
 */
@WebServlet(name = "ViewSerie", urlPatterns = {"/ViewSerie"})
public class ViewSerie extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String search = "";
        int entry = 10;
        int count = dao.countSearch(dao.getAllProductByFilter(filter, search),serieID);
...
//        List<ProductSeri> serieList = dao.getAllProductSerieByFilter2(filter, search, index);
        List<Product> productList = dao.searchProductBy(dao.getAllProductByFilter(filter, search), index, "", entry, serieID);
        request.setAttribute("currentPage", index);
        request.setAttribute("endPage", endPage);
        request.setAttribute("listSize", count);
        request.setAttribute("productList", productList);
        request.getRequestDispatcher("admin/product/temp.jsp").forward(request, response);
    }
